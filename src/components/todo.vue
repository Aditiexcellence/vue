            ...
            <v-flex xs12 sm8 offset-sm2>
                <v-card dark>
                    ...
                    <v-container>
                        ...
                        <v-flex xs12>
                            <v-text-field clearable color="white" v-model="newTodo" id="newTodo" name="newTodo" label="Add task"></v-text-field>
                        </v-flex>
                        <v-layout row wrap>
                            <v-flex xs5>
                            ...
                            <v-spacer></v-spacer>
                            <v-flex xs5>
                                <v-menu ref="menu1" v-model="menu3" :close-on-content-click="false" :nudge-right="40" :return-value.sync="time" lazy transition="scale-transition" offset-y full-width max-width="290px" min-width="290px">
                                    <template v-slot:activator="{ on }">
                                        <v-text-field v-model="time" label="Select Time" prepend-icon="access_time" readonly v-on="on"></v-text-field>
                                    </template>
                                    <v-time-picker v-if="menu3" v-model="time" full-width @click:minute="$refs.menu1.save(time)"></v-time-picker>
                                </v-menu>
                            </v-flex>
                        </v-layout>
                    ...
                    <v-data-table :headers="headers" :items="todolist" class="elevation-1" hide-actions v-if="canShowtable">
